  ...
  return (
    <>
      ...
      <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[1000] bg-[#091018]/95 backdrop-blur-md border border-white/12 rounded-full px-2.5 py-1 flex items-center gap-1 shadow-[0_12px_36px_rgba(0,0,0,0.6)] text-slate-200">
        ...
        <button
          onClick={() => {if(!useMapStore.getState().selectedFeatureId&&currentProject.features[0])setSelectedFeatureId(currentProject.features[0].properties.id);setActivePanel(activePanel === "attributes" ? null : "attributes")}}
          className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
        ...
        <button onClick={()=>window.dispatchEvent(new CustomEvent("atlas:export-png"))} className="w-8 h-8 rounded-full flex items-center justify-center text-slate-400 hover:text-white hover:bg-white/10" title="Export visible map to PNG"><Camera className="w-3.5 h-3.5"/></button>
        <button onClick={()=>supabase.auth.signOut()} className="w-8 h-8 rounded-full flex items-center justify-center text-slate-400 hover:text-rose-300 hover:bg-white/10" title="Sign out"><LogOut className="w-3.5 h-3.5"/></button>
      </div>
    </>
  );
}
